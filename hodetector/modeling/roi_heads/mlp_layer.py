...
class FastRCNNFCHead(nn.Module):
    """
    A head for hand-first object interaction and tool-second object prediction
    """
    ...
    def __init__(
        ...
        input_size = 2057
        self.relation_layer = nn.Sequential(nn.Linear(input_size, 1024), nn.ReLU(), nn.Linear(1024, 1024), nn.ReLU(), nn.Linear(1024, 5))
    ...
    def losses(self, relation_pred, label):
        ...
        losses = {
                    "loss_relation": 0.1 * self.relation_loss(relation_pred, label)
            }
